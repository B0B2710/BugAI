...
def get_parms_for_tool(rules_text, tool):
    ...
    con =bardcode.get_answer(f'**Important: Based on the scope and rules:"{rules_text}", generate parameters for the **{tool}** tool.* The parameters should be in the following format: {tool}: (the parameters for the command) .* You can refer to the target as `"$domain"` *the target must always be inculded in the command*.* the output of the tools will be to a folder on the Desktop named output and the name of the file is tool_name.txt ,the only wordlists that are allowed to use are thoses that 100% installed with the tools used, the error : {error_content} was the output of {broken_command} *pls fix the command* . pls fix the error given.* Always comply with the rules.* Do not explain anything.* Double-check that the command parameters follow the stated rules.')

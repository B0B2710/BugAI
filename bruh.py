...
def get_parms_for_tool(rules_text, tool):
    con =bardcode.get_answer(f'The previous prompt:("Important: Generate parameters for the {tool} tool based on the scope and rules "{rules_text}".The parameters should be in the following format:{tool}: (the parameters for the command).You can refer to the target as "$domain".The target must always be included in the command.The output of the tools will be saved to ~/Desktop/output/ and the name of the file will be "tool_name". The double quotes are important.Only wordlists that allowed is  /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt.*check that all the wordlists are correct* .Always comply with the rules.Do not explain anything.Double-check that the command parameters follow the stated rules.") generated the following command, which did not work: {broken_command}, The error message was: {error_content} To fix this, please: 1. Re-generate the command parameters based on the scope and rules specified in the previous prompt with any other required parameters . 2. Make sure the parameters are in the correct format. 4. the parameters for the command . 5. You can refer to the target as "$domain". 6. The output of the tools should be saved to a folder on the Desktop named "output" with the file name "tool_name.txt". 7. Only use wordlists that are 100% installed with the tools used. 8. Double-check that the command parameters follow the stated rules.')

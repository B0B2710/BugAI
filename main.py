...
def get_parms_for_tool(rules_text, tool):
    payload["input_text"] = f'Important: Generate parameters for the {tool} tooland rules "{rules_text}".The parameters should be in the following format:{tool}: (the parameters for the command).You can refer to the target as "$domain".The target must always be included in the command.add sudo before the command if needed.dont add proxy.The output of the tools will be saved to ~/Desktop/output/ and the name of the file will be "tool_name.txt". The double quotes are important.Only wordlists that are 100% installed with the tools used are allowed to be used.Include the custom HTTP header: X-Hackerone: bobus2710 with every requests that you send to Tide..Always comply with the rules.Do not explain anything.Double-check that the command parameters follow the stated rules.' 
